...
| CHRONICLE | `/chronicle` | 主页面 | 参考 `/Users/tingyouzhao/Desktop/deaho官网/CHRONICLE案例`，详细规格见 `project-docs/10_chronicle.md` |
...
详细规格见：`/Users/tingyouzhao/Desktop/deaho官网/project-docs/10_chronicle.md`。
...
- 左侧固定年份导航。
- 底部固定进度线。
- 四个板块对应 `1994 / 2001 / 2012 / 2026`。
- 支持 day / night 两套主题，并继承全站 `ThemeProvider`。
- Mobile 改为纵向时间线，仍保留年份导航。
...
- 路由：`/news`
- 详情路由：`/news/[slug]`
- 实现文件：`frontend/components/news/NewsPage.tsx`
- 详情实现文件：`frontend/components/news/NewsArticlePage.tsx`
- 数据文件：`frontend/data/news.ts`
- 顶部栏复用 `frontend/components/layout/SiteHeader.tsx`。
- 支持 day / night 两套主题，并继承全站 `ThemeProvider`。
- Day mode 使用 Home day 的冷白、浅蓝和品牌蓝。
- Night mode 使用深色档案感和 muted gold。
- Mobile 使用全站统一 mobile menu，并调整内容间距避免 MENU 覆盖筛选栏。

### Section 1 — Featured Article

- 顶部居中 `NEWS` 标题、细线与 diamond 装饰。
- 背景使用低透明 blueprint 图纸氛围。
- Featured story 使用左右分栏：左侧大图、右侧文章信息。
- 当前 featured story：`Designed to Be Remembered`。
...
- 参考分类：
  ...
  - Events
  - Projects
- 当前 active 状态在 day mode 使用品牌蓝，在 night mode 使用 muted gold。
- 右侧包含 `Latest / Oldest` 排序切换。

### Section 3 — Article Grid

- 3 列卡片 grid。
- 卡片包含图片、标签、日期、标题、摘要和 Read more。
- 当前内容先使用本地 mock data，后续替换真实新闻即可。
- Mobile 改为单列卡片。

### Section 4 — Article Detail

- 点击 featured story、文章图片、文章标题或 `Read more` 后进入对应 `/news/[slug]` 详情页。
- 详情页沿用 NEWS 的 editorial / blueprint 风格，不做普通博客模板。
- 顶部包含返回 NEWS、分类、日期、阅读时长、文章标题和摘要。
- 中段使用大图 hero、图注、左侧 article notes、右侧正文内容。
- 正文内容由 `frontend/data/news.ts` 的 `body / notes / quote / readingTime` 管理。
- 底部展示 3 个 related stories，优先推荐同分类文章。
- Day mode 使用品牌蓝和冷白背景；Night mode 使用 muted gold 与深色档案背景。
- Mobile 下正文改为单列，图注移到图片顶部，避免被固定 MENU 遮挡。
